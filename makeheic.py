...
class makeheic:
    ...
    def run_probe(self):
        probe = subprocess.Popen(r'ffprobe -hide_banner{MED} -i "{INP}"'.format(INP=self.in_fp,MED=' -f concat -safe 0' if self.medium_img else ''),shell=True,stderr=subprocess.PIPE)
    ...
    def cmd_line_gen(self):
        ...
        if not self.isseq:
            ...
            brand='hevc' if ff_pixfmt=='yuv420p' else 'hevx'
        ...
        if not self.isseq:
            if not self.grid or (self.items==1 and not self.gridF):
                ...
                self.ff_cmd_img=r'ffmpeg -hide_banner{HWD} -r 1{MED} -i "{INP}" -vf pad={PW}:{PH},untile={UNT},setpts=N/TB,fps=1,{SF},format={PF} -fps_mode vfr -c:v libx265 -preset 6 -crf {Q} -x265-params keyint=1:sao={SAO}:ref=1:rc-lookahead=0:bframes=0:aq-mode=1:psy-rdoq={PRDO}:cbqpoffs={CO}:crqpoffs={CO}:range=full:colormatrix={MAT_L}:transfer=iec61966-2-1:no-info=1{XP} "{TMPF}\make.heic.{PID}.hevc"{TMBN} -y'
                ...
                self.ff_cmd_a=r'ffmpeg -hide_banner{HWD} -r 1{MED} -i "{INP}" -vf {SF},pad={PW}:{PH},untile={UNT},setpts=N/TB,fps=1,format={PF} -fps_mode vfr -c:v libx265 -preset 6 -crf {Q} -x265-params keyint=1:sao={SAO}:ref=1:rc-lookahead=0:bframes=0:aq-mode=1:psy-rdoq={PRDO}:cbqpoffs={CO}:crqpoffs={CO}:range=full:colormatrix={MAT_L}:transfer=iec61966-2-1:no-info=1{XP} "{TMPF}\make.heic.{PID}.hevc" -y -map v:0 -vf {SFA}pad={PW}:{PH},untile={UNT},setpts=N/TB,fps=1,format={PF2} -fps_mode vfr -c:v libx265 -preset 6 ' + alpha_quality_control + r'-x265-params keyint=1:sao={SAO}:ref=1:rc-lookahead=0:bframes=0:aq-mode=1:psy-rdoq={PRDO}:cbqpoffs=1:crqpoffs=1:range=full:colormatrix={MAT_A}:transfer=iec61966-2-1:no-info=1{XPAL}{XP} "{TMPF}\make.heic.alpha.{PID}.hevc"{TMBN} -y'
...
########################################
        ...
        if not self.isseq:
            if not self.grid or (self.items==1 and not self.gridF):
                ...
                self.ff_cmd_img=r'ffmpeg -hide_banner{HWD} -r 1{MED} -i "{INP}" -vf pad={PW}:{PH},untile={UNT},setpts=N/TB,fps=1,{SF},format={PF} -fps_mode vfr -r 1 -c:v {HWE} -color_range pc -colorspace {MAT_L} -bf 0 -g 1 -qp {Q} "{TMPF}\make.heic.{PID}.hevc"{TMBN} -y'
                ...
                self.ff_cmd_a=r'ffmpeg -hide_banner{HWD} -r 1{MED} -i "{INP}" -vf {SF},pad={PW}:{PH},untile={UNT},setpts=N/TB,fps=1,format={PF} -fps_mode vfr -r 1 -c:v {HWE} -color_range pc -colorspace {MAT_L} -bf 0 -g 1 -qp {Q} "{TMPF}\make.heic.{PID}.hevc" -y -map v:0 -vf {SFA}pad={PW}:{PH},untile={UNT},setpts=N/TB,fps=1,format={PF2} -fps_mode vfr -r 1 -c:v {HWE} -color_range pc -colorspace {MAT_L} -bf 0 -g 1 ' + alpha_quality_control + r'"{TMPF}\make.heic.alpha.{PID}.hevc"{TMBN} -y'
            ...
            return True
########################################
        self.ff_cmd_img=self.ff_cmd_img.format(INP=self.in_fp,PD=pad,SF=scale_filter,Q=self.crf,MAT_L=self.mat_l,PF=ff_pixfmt,CO=coffs,PID=self.pid,SAO=sao,PRDO=prdo,XP=self.xp,TMPF=self.temp_folder,HWD=hwd,HWE=self.hwenc,PW=self.g_padded_w,PH=self.g_padded_h,UNT=str(self.g_columns)+'x'+str(self.g_rows),TMBN=tmbn,MED=' -f concat -safe 0' if self.medium_img else '')
        self.m4b_cmd_img=self.m4b_cmd_img.format(OUT=self.out_fp,ICC=icc_opt,PID=self.pid,WxH=str(int(self.probe_res_w*self.scale[0]))+'x'+str(int(self.probe_res_h*self.scale[1])),BN=brand,TMPF=self.temp_folder,GS=str(self.g_rows)+'x'+str(self.g_columns),REFS=refs,TMBN=tmbn_m4b,TMBD=tmbn_del)

        self.ff_cmd_a=self.ff_cmd_a.format(INP=self.in_fp,PD=pad,SF=scale_filter,Q=self.crf,MAT_L=self.mat_l,PF=ff_pixfmt,CO=coffs,PID=self.pid,PF2=ff_pixfmt_a,Q2=self.acrf,SAO=sao,PRDO=prdo,XP=self.xp,TMPF=self.temp_folder,HWD=hwd,SFA=scale_filter_a,HWE=self.hwenc,PW=self.g_padded_w,PH=self.g_padded_h,UNT=str(self.g_columns)+'x'+str(self.g_rows),TMBN=tmbn,MAT_A=self.mat_l if not self.rgb_color else self.mat_a,MED=' -f concat -safe 0' if self.medium_img else '')
        self.m4b_cmd_a=self.m4b_cmd_a.format(OUT=self.out_fp,PID=self.pid,ICC=icc_opt,WxH=str(int(self.probe_res_w*self.scale[0]))+'x'+str(int(self.probe_res_h*self.scale[1])),BN=brand,TMPF=self.temp_folder,GS=str(self.g_rows)+'x'+str(self.g_columns),REFS=refs,GID=self.items+1,REFS2=refs2,TMBN=tmbn_m4b,TMBD=tmbn_del)
        return True
########################################
    ...
    def encode(self):
        ...
        if self.medium_img:
            ...
            for p in pathlib.Path(self.in_fp).parent.glob(f'make.heic.{self.pid}.*.pam'):
                os.remove(p)
